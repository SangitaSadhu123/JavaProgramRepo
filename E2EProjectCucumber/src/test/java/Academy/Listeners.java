...
public class Listeners extends Base implements ITestListener {
	...
	ThreadLocal<ExtentTest> extentTest=new ThreadLocal<ExtentTest>(); // using ThreadLocal making framework as thread safe sothat we can avoid overried problem of test instance
	...
	public void onTestStart(ITestResult result) {
		...
		extentTest.set(test);                                       // so putting all the test into extentTest of ThreadLocal then we will call
		                                                            // one by one test instance from extentTest
	}
...
// wherever ant test fail control will come to this block and capture the failure
	...
	{
		extentTest.get().fail(result.getThrowable()); // using getThrowable() will get the logs 
		...
		try {
			...
			extentTest.get().addScreenCaptureFromPath(getScreenShot(testMethodName,driver),result.getMethod().getMethodName()); //getScreenShot() calling screenshot method
